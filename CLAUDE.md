# Claude Code Project Instructions — regex-kotlin

## Project Overview

This is **regex-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`regex`](https://crates.io/crates/regex).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/regex/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/regex>
